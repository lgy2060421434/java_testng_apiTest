...
import pojo.HttpGetWithEntity;


public class BodyGet {
    public static Logger logger = Logger.getLogger(BodyPost.class);
    //    存储cookies的键值对
    ...
    public static Map<String, String> doGet(String url, String params, String isNeedToken, String CaseId) {
            ...
            if (isNeedToken.trim().length() > 0 && isNeedToken != null) {
                if (!isNeedToken.substring(0, 1).equals("$")) {
                    ...
                    if (isNeedToken_int > 0) {
                        addTokenBeforeResponse(httpGetWithEntity, isNeedToken);
                    }
                }
            }

//			在发送请求前,判断是否有cookie
            addCookieBeforRespons(httpGetWithEntity);
            HttpEntity httpEntity  = new StringEntity(jsonObject.toJSONString(), ContentType.APPLICATION_JSON);
